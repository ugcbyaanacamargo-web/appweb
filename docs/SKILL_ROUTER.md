...
| Architecture / module boundaries / APIs | gstack `plan-eng-review` | Addy `api-and-interface-design`; Superpowers planning | Explicit contracts, data flow, failure modes, test strategy |
...
| Persist continuity | gstack `context-save` | repository `PROJECT_STATE.md` + `DECISIONS.md` | Durable factual checkpoint after meaningful work |
...
| Security | gstack `cso` | Addy `security-and-hardening` | Evidence-backed security findings and fixes |
...
| Browser / end-to-end QA | gstack `qa` / `qa-only` | Addy `browser-testing-with-devtools` | Real behavior verified with available browser/runtime tools |
